...
class CameraTests: XCTestCase {
    func testPixelSizeForHorizontalCanvas() async throws {
        let camera = Camera(width: 200,
                            height: 125,
                            viewAngle: PI/2,
                            viewTransform: .identity)
        let actualValue = await camera.pixelSize
        let expectedValue = 0.01
        XCTAssert(actualValue.isAlmostEqual(expectedValue))
    }

    func testPixelSizeForVerticalCanvas() async throws {
    ...
    func testRayForPixelForCenterOfCanvas() async throws {
        ...
        let ray = await camera.rayForPixel(100, 50)
    ...
    func testRayForPixelForCornerOfCanvas() async throws {
        let camera = Camera(width: 201, height: 101, viewAngle: PI/2, viewTransform: .identity)
        let ray = await camera.rayForPixel(0, 0)
        XCTAssert(ray.origin.isAlmostEqual(Point(0, 0, 0)))
